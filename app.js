...
const modeDescriptions = {
  taint:
    "Tracks untrusted data flow from sources (user input) to dangerous sinks (SQL, XSS, etc.)",
  regex:
    "Traditional pattern matching to detect vulnerabilities based on code patterns",
  hybrid:
    "Combines both taint analysis and regex patterns for comprehensive detection",
};
...
async function analyzeGithubRepo() {
  ...
  try {
    ...
    if (data.success) {
      currentAnalysisData = data;
      // Add repo info to display
      currentAnalysisData.repoUrl = repoUrl;
      currentAnalysisData.isGithub = true;
      displayResults(data);
    } else {
...
async function analyzeFolder(file) {
  const formData = new FormData();
  formData.append("file", file);
  ...
  try {
    ...
    if (data.success) {
      currentAnalysisData = data;
      currentAnalysisData.isFolder = true;
      currentAnalysisData.folderName = file.name;
      displayResults(data);
    } else {
      showError(data.error || "Folder analysis failed");
    }
  } catch (error) {
    showError("Error analyzing folder: " + error.message);
  } finally {
    hideLoading();
  }
}
...
function displayVulnerabilities(vulnerabilities) {
  ...
  if (vulnerabilities.length === 0) {
    vulnerabilitiesList.innerHTML =
      '<div class="no-results">No vulnerabilities match the selected filters.</div>';
    return;
  }
  ...
  try {
    const response = await fetch(`/api/report/${format}`, {
      ...
      body: JSON.stringify({
        ...
        mode: currentAnalysisData.analysis_mode || "taint",
      }),
    });
